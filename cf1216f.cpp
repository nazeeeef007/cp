...
void solve(int n, int k, string & a) {
    deque<int> d;
    vector<vector<ll>> dp(n+1,vector<ll>(2,1e14));
    dp[0][0] = 0;
    dp[0][1] = 0;
    for (int i = 0; i < n; ++i){
        dp[i+1][0] = dp[i][0] + i+1;
        ...
        if (!d.empty()){
            dp[i+1][0] = min(dp[i+1][0] , dp[d.front()][1]);
        }
        if (a[i] == '1'){
            dp[i+1][1] = dp[i+1][0] + i + 1;
            dp[i+1][1]  =min(dp[i+1][1], i + 1 + min(dp[max(0, i-k)][0],dp[max(0, i-k)][1]));
            while (!d.empty() && dp[i+1][1] < dp[d.front()][1]){
                d.pop_back();
...
// at every room i can just directly connect it to the wifi, 
// or i can put a router there(if s[i] == 1), both for i coins, 
// or i just go through all j from [i-k:i] eg i = 4, k = 1 
// j = 1,2,3, i take the min of dp[j] 
// ok but can  iuse a monotonic increasing deque? 
